...
def _slug_from_link_mistmint(link: str) -> str:
    try:
        segs = [s for s in urlparse(link or "").path.split("/") if s]
        return segs[1] if len(segs) >= 2 and segs[0] == "novels" else ""
    except Exception:
        return ""

def _short_code_from_slug_mistmint(slug: str) -> str:
    block = HOSTING_SITE_DATA.get("Mistmint Haven", {}).get("novels", {}) or {}
    for _title, det in block.items():
        u = (det.get("novel_url") or "").rstrip("/")
        if u and u.split("/")[-1] == slug:
            return (det.get("short_code") or "").strip()
    return ""

def _chapter_number_from_item(it: dict) -> int | None:
    # Try "Chapter 12.5" from chaptername, else fall back to nameextend like "1.2"
    for field in ("chaptername", "nameextend"):
        s = (it.get(field) or "").strip()
        m = re.search(r'(\d+(?:\.\d+)?)', s)
        if m:
            try:
                ...
                pass
    # Final fallback: try parsing from link tail "…-chapter-12" if present
    link = it.get("link") or ""
    tail = (link.rstrip("/").split("/")[-1] if "/" in link else link).lower()
    m = re.search(r'(?:^|-)chapter-(\d+(?:\.\d+)?)\b', tail)
    if m:
        try:
            return int(float(m.group(1)))
        except Exception:
...
async def _fetch_paid_numbers_for_slug(session, slug: str) -> set[int]:
    """Return chapterNumbers that are currently paid (isFree == False)."""
    url = f"{BASE_API}/novels/slug/{slug}/chapters"
    try:
        async with session.get(url, headers=_mistmint_headers(), timeout=AIOHTTP_TIMEOUT) as resp:
            if resp.status != 200:
                diag_fail("mistmint-paidset-http", slug=slug, code=resp.status)
                return set()
            data = await resp.json()
    except Exception as e:
        diag_fail("mistmint-paidset-ex", slug=slug, error=str(e))
        return set()
    out: set[int] = set()
    for vol in (data or {}).get("data", []) or []:
        for ch in (vol.get("chapters") or []):
            if ch.get("isFree") is False and not ch.get("isHidden"):
                try:
                    out.add(int(float(ch.get("chapterNumber") or 0)))
                except Exception:
                    pass
    return out

def _prefer_api(a: dict, b: dict) -> dict:
    pri = {"api": 2, "manual": 1, "history": 0, "": 0, None: 0}
    sa = pri.get((a.get("source") or "").lower(), 0)
    sb = pri.get((b.get("source") or "").lower(), 0)
    if sa != sb:
        return a if sa > sb else b
    # tie-breaker: latest pubDate if both look the same
    da = a.get("pubDate"); db = b.get("pubDate")
    try:
        return a if (da and db and da > db) else (b if (da and db and db > da) else a)
    except Exception:
        return a

async def dedup_and_prune_mistmint_paid(session, items: list[dict]) -> list[dict]:
    """
    - For Mistmint items only:
      * Drop any (manual/history) chapter that is no longer paid per API.
      * Collapse duplicates by (short_code, chapterNumber), prefer API.
      * Normalize GUIDs to '<short_code>-<chapterNumber>'.
    """
    # Group by novel slug found in link
    per_slug: dict[str, list[dict]] = {}
    for it in items:
        link = it.get("link") or ""
        if "mistminthaven.com/novels/" not in link:
            continue
        slug = _slug_from_link_mistmint(link)
        if slug:
            per_slug.setdefault(slug, []).append(it)

    keep_ids = set()
    for slug, group in per_slug.items():
        paid_set = await _fetch_paid_numbers_for_slug(session, slug)
        scode = _short_code_from_slug_mistmint(slug) or slug

        # 1) prune those that are not in current paid set (unless we can’t fetch, then skip pruning)
        pruned = []
        for it in group:
            n = _chapter_number_from_item(it)
            if n is None:
                pruned.append(it)  # keep uncertain ones
                continue
            if paid_set and (n not in paid_set):
                # drop stale manual/history item
                src = (it.get("source") or "").lower()
                diag_ok("mistmint-prune-free", slug=slug, chapter=n, source=src)
                continue
            pruned.append(it)

        # 2) collapse duplicates by canonical key; prefer API
        ...
        for it in pruned:
            ...
            # normalize GUID to stable form so history can't split later
            it["guid"] = f"{scode}-{n}"

            cur = best.get(key)
            best[key] = it if cur is None else _prefer_api(it, cur)

        for chosen in best.values():
            keep_ids.add(id(chosen))

    # Return filtered list preserving original order for everything else
    out = []
    for it in items:
        link = it.get("link") or ""
        if "mistminthaven.com/novels/" in link:
